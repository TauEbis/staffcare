...
feature "Deleting a patient volume forecast: " do
  ...
  given(:locations)  { [ FactoryGirl.create(:location), FactoryGirl.create(:location) ] }
	given!(:earlier_forecast) { FactoryGirl.create(:patient_volume_forecast, start_date: Date.today, locations: locations) }
	given!(:later_forecast) { FactoryGirl.create(:patient_volume_forecast, start_date: Date.today + 14, locations: locations) }
  ...
  describe "Given am admin visits the index page," do
		...
		it { should have_link('Delete', href: patient_volume_forecast_path(earlier_forecast) ) }
